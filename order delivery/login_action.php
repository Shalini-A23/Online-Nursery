...
{
	$sql="SELECT id,status FROM `".TBL_DELIVERY_ADMIN."` where `username`='".$username."' AND `password`='".$password."' ";
      ...
      if($row['status']==1)
  {
    if(isset($_POST['remember']))
    {
      ...
      {
        setcookie ("username_delivery_admin",$username,time()+ 3600);
        echo "Cookies Set Successfuly";
      } 
      else 
      {
        setcookie("username_delivery_admin","");
        echo "Cookies Not Set";
      }
    }
		$_SESSION['delivery_admin_id']=$row[0];
...
{
  $sql="SELECT username FROM `".TBL_DELIVERY_ADMIN."` where `name`='".$_POST['name']."' AND `username`='".$_POST['username']."' AND status=1 ";
  $result = $database->query($sql);  
  $row = $database->fetch_array($result);
  if(mysqli_num_rows($result)==1) 
  {
    ...
    {
      $sql="UPDATE `".TBL_DELIVERY_ADMIN."` SET `password`='".$a."' WHERE username='".$to."' ";
      $result = $database->query( $sql );
      if ($result) 
      {
